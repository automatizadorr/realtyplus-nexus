...
function drawSphere(
  ...
  if (state === "listening") {
    body.addColorStop(0, "#ff8899");
    body.addColorStop(0.45, "#cf142b");
    body.addColorStop(1, "#6b0000");
  } else if (state === "speaking") {
    body.addColorStop(0, "#4da6ff");
    body.addColorStop(0.45, "#0f2b5a");
    body.addColorStop(1, "#040d1e");
  } else {
    body.addColorStop(0, "#e8e8f0");
    body.addColorStop(0.45, "#9090b0");
    body.addColorStop(1, "#404060");
  }
  ctx.beginPath();
  ...
  for (let i = 1; i < latCount; i++) {
    ...
    // Foreshortening based on current rotation angle
  ...
  for (let i = 0; i < lonCount; i++) {
    const lon = (Math.PI * 2 * i) / lonCount + t;
    ...
    for (let j = 0; j <= 60; j++) {
      ...
      const zDepth = Math.cos(lat) * Math.sin(lon); // -1 to 1
      const alpha = (zDepth + 1) / 2; // 0 = back, 1 = front
      ...
      ctx.strokeStyle = wireColor.replace(/[\d.]+\)$/, `${(wireAlpha * alpha + 0.05).toFixed(2)})`);
    }
    ctx.stroke();
  }
  ...
  if (state !== "idle") {
    ...
    ctx.strokeStyle =
      state === "listening" ? "rgba(207,20,43,0.6)" : "rgba(15,43,90,0.7)";
...
function SonarRing({
  ...
  const color =
    state === "listening" ? "#cf142b" :
    state === "speaking"  ? "#0f2b5a" :
                             "#9090b0";
        ...
        return (
          <motion.div
            ...
            animate={{
              height: [
                4 + Math.sin(phase) * 3,
                4 + amp * 48 * (0.4 + Math.abs(Math.sin(phase + Date.now() / 500))) + Math.random() * 4,
                4 + Math.sin(phase) * 3,
              ],
            }}
            transition={{
              repeat: Infinity,
              duration: 0.25 + (i % 5) * 0.06,
              ease: "easeInOut",
            }}
          />
        );
...
export function VoiceAgentHero() {
  ...
  const subLabels: Record<AgentState, string> = {
    ...
    connecting: "Solicitando acceso al micrófono…",
    listening:  "El agente te escucha en tiempo real",
    speaking:   "Respuesta generada por IA en tiempo real",
  };
  const accentColor =
    state === "listening" ? "#cf142b" :
    state === "speaking"  ? "#0f2b5a" :
  ...
  return (
    <section
      id="agente-voz"
      className="relative py-24 px-6 overflow-hidden bg-gradient-to-b from-gray-50 to-white"
    >
      {/* Background blobs */}
      <div
      ...
      <div
        ...
        style={{ background: "#0f2b5a" }}
      />

      <div className="max-w-4xl mx-auto relative z-10">
        {/* Header */}
        <div className="text-center mb-14">
          <span className="text-[#cf142b] text-sm font-bold uppercase tracking-widest block mb-3">
            Agente de voz IA
          </span>
          <h2 className="text-4xl md:text-5xl font-black text-[#040d1e] mb-4">
        ...
        <div className="flex flex-col items-center gap-8">
          <div className="relative" style={{ width: 300, height: 300 }}>
            ...
            <motion.button
              onClick={handleClick}
              className="absolute inset-6 rounded-full focus:outline-none focus-visible:ring-4 cursor-pointer"
              style={{
                focusRingColor: accentColor,
                boxShadow: `0 0 ${24 + amp * 40}px ${amp * 20}px ${accentColor}44`,
              }}
          ...
          <div className="text-center">
            <motion.p
              className="text-lg font-bold text-[#040d1e] mb-1"
              key={state}
            ...
            <motion.p
              className="text-sm text-[#040d1e]/45"
              key={state + "sub"}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
          ...
          <AnimatePresence>
            {state !== "idle" && state !== "connecting" && (
              <motion.button
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 8 }}
                onClick={() => { stopMic(); setState("idle"); triggerWidget(); }}
