...
export function buildProEmail(opts: ProEmailOptions): string {
  ...
  // Botón simple, sin borde dorado ni sombra: cuanto más "banner de
  // marketing" se ve un botón, más chance de caer en Promociones/Spam.
  const primaryBtn = showCta
    ...
    <td align="center" style="border-radius:8px;background:${brand};">
      <a href="${esc(ctaUrl)}" target="_blank" style="display:inline-block;padding:12px 28px;font-family:${FONT_BODY};font-size:15px;font-weight:700;letter-spacing:.2px;color:#ffffff;text-decoration:none;border-radius:8px;background:${brand};">${esc(cta)}</a>
    </td></tr></table>`
    : "";
